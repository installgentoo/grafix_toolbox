...
impl Cast<f32> for i128 {
	fn to(v: f32) -> Self {
		let check = |v: f32| v.trunc() >= i128::min_value() as f32 && v.trunc() <= i128::max_value() as f32;
		ASSERT!(check(v), "Error casting {} to i128", v);
		unsafe { v.to_int_unchecked() }
	}
}
impl Cast<f64> for i128 {
	fn to(v: f64) -> Self {
		let check = |v: f64| v.trunc() >= i128::min_value() as f64 && v.trunc() <= i128::max_value() as f64;
		ASSERT!(check(v), "Error casting {} to i128", v);
		unsafe { v.to_int_unchecked() }
	}
}
