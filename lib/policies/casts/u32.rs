...
impl Cast<f32> for u32 {
	fn to(v: f32) -> Self {
		let check = |v: f32| v.trunc() >= u32::min_value() as f32 && v.trunc() <= u32::max_value() as f32;
		ASSERT!(check(v), "Error casting {} to u32", v);
		unsafe { v.to_int_unchecked() }
	}
}
impl Cast<f64> for u32 {
	fn to(v: f64) -> Self {
		let check = |v: f64| v.trunc() >= u32::min_value() as f64 && v.trunc() <= u32::max_value() as f64;
		ASSERT!(check(v), "Error casting {} to u32", v);
		unsafe { v.to_int_unchecked() }
	}
}
