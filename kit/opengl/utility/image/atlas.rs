...
{
	if tiles.is_empty() {
		return Err("Empty vector supplied to atlas").warn();
	}

	tiles.sort_unstable_by(|(_, l), (_, r)| if l.h() != r.h() { r.h().cmp(&l.h()) } else { r.w().cmp(&l.w()) });
	ASSERT!(
	...
	let max_w = {
		...
		max_w.min(i32(2_u32.pow(u32(f64(area).sqrt().log2() + 1.))))
	};
	let (min_w, min_h) = (tiles.iter().map(|(_, e)| e.w()).min().valid(), tiles.last().map(|(_, e)| e.h()).valid());
	...
	for i in 0..tiles.len() {
		let (id, img) = tiles.at(i).as_valid();

		let duplicate = tiles[..i].iter().rev().flatten().take_while(|(_, e)| e.h() == img.h()).find(|(_, e)| img == e);

		if let Some((id_d, _)) = duplicate {
			packed.insert(id.clone(), *packed.get(id_d).valid());
			DEBUG!("Deduped {id_d:?}, {id:?} in atlas");
			continue;
		}

		let Ok(b) = pack(img.w(), img.h(), empty, filled, (min_w, min_h)) else {
			tail.push(tiles.at_mut(i).take().valid());
			continue;
		};

		let (x, y, w, h) = (b.x, b.y, b.w, b.h);
		packed.insert(id.clone(), (x, y, x + w, y + h));
	...
	let tex = Tex2d::new((max_w, max_h), &atlas[..]).pipe(Rc::new);
	let packed = packed
		.into_iter()
		.map(|(id, reg)| {
			let region = Vec4(reg).div(tex.whdl().xyxy());
