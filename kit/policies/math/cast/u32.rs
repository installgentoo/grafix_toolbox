...
impl Cast<f32> for u32 {
	fn to(v: f32) -> Self {
		ASSERT!(v.trunc() >= u32::min_value() as f32 && v.trunc() <= u32::max_value() as f32, "Error casting {v} to u32");
		unsafe { v.to_int_unchecked() }
	}
}
impl Cast<f64> for u32 {
	fn to(v: f64) -> Self {
		ASSERT!(v.trunc() >= u32::min_value() as f64 && v.trunc() <= u32::max_value() as f64, "Error casting {v} to u32");
		unsafe { v.to_int_unchecked() }
	}
}
