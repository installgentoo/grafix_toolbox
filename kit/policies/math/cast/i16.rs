...
impl Cast<f32> for i16 {
	fn to(v: f32) -> Self {
		ASSERT!(v.trunc() >= i16::min_value() as f32 && v.trunc() <= i16::max_value() as f32, "Error casting {v} to i16");
		unsafe { v.to_int_unchecked() }
	}
}
impl Cast<f64> for i16 {
	fn to(v: f64) -> Self {
		ASSERT!(v.trunc() >= i16::min_value() as f64 && v.trunc() <= i16::max_value() as f64, "Error casting {v} to i16");
		unsafe { v.to_int_unchecked() }
	}
}
